...
//const utils_1 = require("./pp_utils");
...
(function (PreciseProofs) {
    ...
    PreciseProofs.hash = (input) => {
        return ethereumjs_util_1.bufferToHex(ethereumjs_util_1.keccak(input)).substr(2);    // crypto-heimdall .. keccak <- sha3
    };
    ...
    PreciseProofs.findLeaf = (leafs, key) => {
    ...
    PreciseProofs.createProof = (key, leafs, withSchema, existingMerkleTree) => {
        ...
        const leafToProve = PreciseProofs.findLeaf(sortedLeafs, key);
        const proof = {
            key: key,
    ...
    const pathForPostion = (schemaLength, position) => {
        ...
        const theLeaf = PreciseProofs.findLeaf(positions, position.toString());
